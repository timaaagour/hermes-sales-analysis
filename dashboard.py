...
Pour lancer : python dashboard.py
...
regions    = {"Chine": 0.222, "Europe": 0.200, "Japon": 0.155,
              "Amériques": 0.145, "Asie-Pacifique": 0.138,
              "Moyen-Orient": 0.080, "Autres": 0.060}
categories = {"Maroquinerie": 0.42, "Soie & Textiles": 0.18,
              "Prêt-à-Porter": 0.15, "Parfums & Beauté": 0.10,
              "Horlogerie": 0.09, "Joaillerie": 0.06}
...
df = pd.DataFrame({
    "date": dates, "client_id": client_ids,
    "region": region_list, "category": category_list,
    "product": product_list, "amount": amounts,
...
children=[
    ...
    html.Div(style={"maxWidth": "1400px", "margin": "0 auto", "padding": "30px 20px"},
    children=[
        ...
        children=[
            html.Div(dcc.Graph(id="monthly-chart"), style=CARD_STYLE),
            html.Div(dcc.Graph(id="category-chart"), style=CARD_STYLE),
        ]),
        ...
        html.Div(style={"display": "grid", "gridTemplateColumns": "1fr 1fr",
                        "gap": "20px", "marginBottom": "20px"},
        children=[
            html.Div(dcc.Graph(id="region-chart"), style=CARD_STYLE),
            html.Div(dcc.Graph(id="season-chart"), style=CARD_STYLE),
        ]),
        ...
        html.Div(style={"display": "grid", "gridTemplateColumns": "1fr 1fr",
                        "gap": "20px", "marginBottom": "20px"},
        children=[
            html.Div(dcc.Graph(id="products-chart"), style=CARD_STYLE),
            html.Div(dcc.Graph(id="rfm-chart"), style=CARD_STYLE),
        ]),

        # Footer
        html.Div(style={"textAlign": "center", "padding": "20px",
                        "color": GRAY, "fontSize": "11px", "letterSpacing": "1px"},
        children=[
            html.P("Données simulées à des fins analytiques · Fatima Aagour · Data Analyst · aagour.fati@gmail.com"),
            html.P("Les données présentées sont entièrement simulées. Ce projet n'est pas affilié à Hermès International.",
                   style={"color": LIGHT}),
        ]),
    ]),
])
...
LAYOUT_BASE = dict(
    ...
    yaxis=dict(showgrid=True, gridcolor=LIGHT, linecolor=LIGHT),
)
...
def update_all(year, region):
    d = filter_df(year, region)

    # KPIs
    ca     = d["amount"].sum()
    panier = d["amount"].mean()
    ...
    def kpi_card(val, label, sub=""):
        return html.Div(style=METRIC_STYLE, children=[
            html.P(label, style={"color": GRAY, "fontSize": "11px",
                                 "letterSpacing": "1px", "margin": "0 0 8px"}),
            html.P(val,   style={"color": DARK, "fontSize": "26px",
                                 "fontWeight": "600", "margin": "0"}),
            html.P(sub,   style={"color": GOLD, "fontSize": "11px", "margin": "4px 0 0"}),
        ])

    kpis = [
        kpi_card(f"{ca/1e6:.1f}M€", "Chiffre d'affaires", "2022 – 2024"),
        kpi_card(f"{panier:,.0f}€",  "Panier moyen", "par transaction"),
        kpi_card(f"{n_cli:,}",       "Clients actifs", "sur la période"),
        kpi_card(top_r,              "Top région", f"{top_ca/1e6:.1f}M€"),
    ]

    # Monthly
    monthly = d.groupby(["year","month"])["amount"].sum().reset_index()
    fig_m = go.Figure()
    year_colors = {2022:"#8B7355", 2023:GOLD, 2024:DARK}
    for y, col in year_colors.items():
        md = monthly[monthly["year"]==y].sort_values("month")
        if len(md):
            fig_m.add_trace(go.Scatter(
                x=md["month"], y=md["amount"]/1e6, mode="lines+markers",
                name=str(y), line=dict(color=col, width=2.5),
                marker=dict(size=6), fill="tozeroy",
                fillcolor=col.replace("#","rgba(").rstrip(")") + ",0.06)" if col.startswith("#") else col
            ))
    fig_m.update_layout(**LAYOUT_BASE, title="Évolution mensuelle du CA",
                        xaxis=dict(tickvals=list(range(1,13)),
                                   ticktext=["Jan","Fév","Mar","Avr","Mai","Jun",
                                             "Jul","Aoû","Sep","Oct","Nov","Déc"],
                                   showgrid=False, linecolor=LIGHT),
                        yaxis=dict(ticksuffix="M€", showgrid=True,
    ...
    fig_c.update_layout(**LAYOUT_BASE, title="Répartition par catégorie",
                        showlegend=True,
    ...
    fig_r  = go.Figure(go.Bar(
        ...
        text=[f"{v:.1f}M€" for v in reg_ca.values/1e6],
        textposition="outside", hovertemplate="%{y}<br>%{x:.1f}M€<extra></extra>",
    ))
    fig_r.update_layout(**LAYOUT_BASE, title="CA par région",
                        xaxis=dict(ticksuffix="M€", showgrid=True,
                                   gridcolor=LIGHT, linecolor=LIGHT),
                        yaxis=dict(showgrid=False, linecolor=LIGHT))

    # Season
    season_order = ["Printemps","Été","Automne","Hiver"]
    sea_ca = d.groupby("season")["amount"].sum().reindex(season_order).fillna(0)
    fig_s  = go.Figure(go.Bar(
        x=sea_ca.index, y=sea_ca.values/1e6,
        marker=dict(color=[GOLD,"#8B7355",DARK,LIGHT],
                    line=dict(color=WHITE, width=1)),
        text=[f"{v:.1f}M€" for v in sea_ca.values/1e6],
        textposition="outside",
    ))
    fig_s.update_layout(**LAYOUT_BASE, title="Saisonnalité",
                        yaxis=dict(ticksuffix="M€", showgrid=True,
                                   gridcolor=LIGHT, linecolor=LIGHT))
    ...
    fig_p = go.Figure(go.Bar(
        x=top_p.values/1e6, y=top_p.index[::-1][::-1],
        orientation="h",
        marker=dict(color=bar_colors[::-1], line=dict(color=WHITE, width=0.5)),
        text=[f"{v:.1f}M€" for v in top_p.values/1e6],
        textposition="outside",
    ))
    fig_p.update_layout(**LAYOUT_BASE, title="Top 8 produits",
                        xaxis=dict(ticksuffix="M€", showgrid=True,
                                   gridcolor=LIGHT, linecolor=LIGHT))
    ...
    fig_rfm.update_layout(**LAYOUT_BASE, title="Segmentation RFM — K-Means",
                          xaxis=dict(title="Récence (jours)", showgrid=False, linecolor=LIGHT),
                          yaxis=dict(title="Valeur (k€)", ticksuffix="k€",
                                     showgrid=True, gridcolor=LIGHT, linecolor=LIGHT),
                          legend=dict(orientation="h", y=1.1))
